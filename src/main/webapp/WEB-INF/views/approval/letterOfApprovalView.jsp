					...
					<table>
							...
							<tr class="signImg">
								<c:choose>
									...
									<c:when test="${approval.appPresent eq 'C'}">
										<td name="firstA" id="firstA">${approval.firstApprover}
											<img src="${path}/images/approved.png"/>
										</td>
						...
						<tbody>
							...
							<tr class="contentarea">
								<td colspan="8">
									<textarea name="loaContent" id="loaContent" cols="151px" rows="7px" readonly>${approval.loaContent}</textarea>
								</td>
							</tr>
							<tr>
					...
					<div id="button">
						...
						<c:choose>
							<c:when
								...
								<button type="submit" id="canceldone"
									onclick="showCancelForm(${approval.appNo}, '${approval.appPresent}')">반려</button>
							</c:when>
							<c:otherwise>
						...
						<button onclick="location.href='${path}/approval/approvalMain'">취소</button>
					</div>
				</div>
